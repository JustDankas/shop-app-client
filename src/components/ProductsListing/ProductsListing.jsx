...
function ProductsListing({data}) {
    ...
    const renderProducts = data?.map((product,index)=>(
        <li key={index} className="product-c">
            ...
            <Link className='product-img-link' to={`/products/${product._id}/${product.title.replace(new RegExp(/[\s\/]/,'gi'),'-')}`} >
                <img src={product.image} alt="" className="product-image" />
            </Link>

            </div>
            <div className="product-info">
                <Link to={`/products/${product._id}/${product.title.replace(new RegExp(/[\s\/]/,'gi'),'-')}`} className="product-title">{product.title}</Link>
                ...
                <div className="product-price"><span>from</span> {product.price} $</div>
            </div>

            <button className="fav-btn" onClick={()=>AddToFav(product._id)}>
                {favs?.includes(product._id)===false?<AiOutlineHeart className='fav-icon'/>:<AiFillHeart className='fav-icon-active' />}
            </button>
        </li>
    ))
