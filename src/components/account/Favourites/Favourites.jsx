                ...
                {favs?.map((product,index)=>(
                    <li key={index} className="product-c">
                        ...
                        <button className="fav-btn" onClick={()=>AddToFav(product._id)}>
                            {favsId?.includes(product._id)===false?<AiOutlineHeart className='fav-icon'/>:<AiFillHeart className='fav-icon-active' />}
                        </button>
                    </li>
                ))}
            </ul>}
            {favs.length==0 && <h3 className='missing-favs-heading'>Explore our products and add them to your favourite wishlist!</h3>}
        </div>
     );
}
