    ...
    return ( 
        <div className="cart-c">
            ...
            >
                ...
                {!activeCart && cartObj.length>0 && <div className="items-count-circle">{cartObj.length}</div>}
            </button>
            ...
            <div className="cart-list-c">
                ...
                <ul className="cart-list">
                    ...
                    {cartObj.map((item,index)=>(
                        <li key={index} className="item-c">
                            <Link to={`/products/${item.product._id}/${item.product.title.replace(new RegExp(/[\s\/]/,'gi'),'-')}`} 
                ...
                {cartObj?.length===0 && <div className="cart-center-div">
                    <RiGhostLine className='ghost-icon' />
                </div>}
                
                <div className="cart-center-div">
                    <Link className='checkout-link' to={'/account/my-account/cart'}>Checkout cart</Link>
                </div>
            </div>
            }
        </div>
