...
function Admin() {
  ...
  const renderAddFilters = categoryFilters.map((filter, index) => (
    <div className="new-category-filter">
      <div className="field">
        <label htmlFor={`filter-name-input-${index}`} className="form-label">
          Filter Name:
        </label>
        {actionType === "add" && (
          <input
            ...
            id={`filter-name-input-${index}`}
            onChange={(e) => handleFilterNameChange(e.target.value, index)}
          />
        )}
        {actionType === "update" && (
          <div className="admin-input-disabled">{filter.filterName}</div>
        )}
      </div>
      <label className="form-label">
        ...
        <input
          type="checkbox"
          className="admin-input-checkbox"
        ...
        <div className="filter-values-list">
          {categoryFilters[index]?.values.map((value, col) => {
            return (
              <ResizableInput
                paramValue={value}
                UpdateText={(text) => handleFilterValueChange(text, index, col)}
              />
            );
          })}
          <button
            type="button"
            className="add-filter-value-btn"
            onClick={() => handleAddFilterValue(index)}
          >
            <MdOutlineAdd className="add-filter-icon" />
          </button>
        </div>
  ...
  const renderProducts = data
    ...
    .map((product) => (
      <div className="admin-product-main-c">
        <button
          key={product._id}
          className="admin-product-c"
          onClick={() => handleItemInfoFill(product)}
        >
          ...
          >
            <RiDeleteBin7Line className="delete-product-icon" />
          </button>
        </button>
        <div className="desc-c">{product.description}</div>
      </div>
    ));
  ...
  function handleItemInfoFill(product) {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/category/${product.categories}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  function handleItemDelete(id) {
    ...
    fetch(
      `${process.env.REACT_APP_API_URL}/admin/product/delete/${id}`,
      requestOptions
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  function handleProductFormSubmit(e) {
    ...
    fetch(
      `${process.env.REACT_APP_API_URL}/admin/product/${
        actionType == "add" ? "new" : `update/${productId}`
      }`,
      requestOptions
    )
      .then((res) => res.json())
  ...
  function handleCategoryFormSubmit(e) {
    ...
    fetch(
      `${process.env.REACT_APP_API_URL}/admin/category/${
        actionType == "add" ? "new" : `update/${valuesArr[0].value}`
      }`,
      requestOptions
    )
      .then((res) => res.json())
  ...
  function handleDeleteCategory(name) {
    if (name) {
      axios
        .delete(
          `${process.env.REACT_APP_API_URL}/admin/category/delete/${name}`,
          {}
        )
        .then((res) => {
          console.log(res);
  ...
  return user.isAdmin && innerWidth > 730 ? (
    <div className="admin">
      <div className="admin-c">
        <div className="admin-type-buttons-c">
          <div className="type-buttons">
            <button
              className={formType === "product" ? "selected-btn" : ""}
            ...
            <button
              ...
              className={actionType === "add" ? "selected-btn" : ""}
            >
              Add
            </button>
            -
          ...
          {formType == "product" && (
            <label
              htmlFor="search-admin-input"
              className="search-product-label"
            >
              Search Item:
              <input
                type="text"
                value={search.get("item")}
                onChange={(e) => setSearch({ item: e.target.value })}
                className="admin-input"
        ...
        <div className="admin-form-preview-c">
          <div className="admin-form-c">
            {formType == "product" && (
              ...
              >
                <div className="field">
                  <label htmlFor="title-admin-input" className="form-label">
                    Product Title:
                  </label>
                  <TextArea
                    value={productTitle}
                    id="title-admin-input"
                    updateInput={(text) => setProductTitle(text)}
                  />
                </div>
                <div className="field">
                  <label htmlFor="price-admin-input" className="form-label">
                    Price:
                  </label>
                  <input
                    type="text"
                    className="admin-input"
                    id="price-admin-input"
                    value={productPrice}
                    onChange={(e) => setProductPrice(e.target.value)}
                  />
                </div>
                <div className="field">
                  <label htmlFor="image-admin-input" className="form-label">
                    Image:
                  </label>
                  <input
                    type="text"
                    className="admin-input"
                    id="image-admin-input"
                    value={productImage}
                ...
                <div className="field">
                  <label htmlFor="descr-admin-input" className="form-label">
                    Description:
                  </label>
                  <TextArea
                    value={productDesc}
                    id="descr-admin-input"
                    updateInput={(text) => setProductDesc(text)}
                  />
                </div>
                <label style={{ marginTop: "2rem" }} className="form-label">
                  Product Category:
                </label>
                <SelectBox
                  value={productCategory}
                  optionsArray={categoryData?.categories}
                  updateCategory={(category) =>
                    handleUpdateProductCategory(category)
                  }
                />
                <ul className="filters-list">
                  {currentProductCategory?.filters?.map((filter, index) => {
                    return (
                      <li key={index} className="admin-filter-c">
                        {filter.filterName}
                        {filter.values.map((v) => (
                          <div className="admin-filter-option">
                            <label
                              htmlFor={`checkbox-${filter.filterName}-${v}`}
                            >
                              {v}
                            </label>
                            {
                              <input
                                ...
                                checked={
                                  productFilters[index]?.values.hasOwnProperty(
                                    v
                                  )
                                ...
                                onChange={(e) =>
                                  handleProductFilterChange(e, v, index)
                                }
                              />
                            }
                ...
                {actionType == "add" && (
                  <button className="submit-add-btn submit-btn" type="submit">
                    <MdOutlineLibraryAdd className="add-icon" /> Add Product
                  </button>
                )}
                {actionType == "update" && (
                  <button
                    className="submit-update-btn submit-btn"
                    type="submit"
                  >
                    <MdSystemUpdateAlt className="update-icon" /> Update
                  </button>
                )}
              </form>
            )}
            ...
            {formType == "category" && (
              <div className="category-selectbox-c">
                <SelectBox
                  optionsArray={categoryData?.categories}
                  updateCategory={(category) =>
                    handleFillCategoryInfo(category)
                  }
                />
              </div>
            )}
            {formType == "category" && (
              ...
              >
                ...
                <div className="field">
                  <label htmlFor="category-image-input" className="form-label">
                    Image:
                  </label>
                  <input
                    type="text"
                ...
                <div className="field">
                  <label
                    htmlFor="category-keywords-textarea"
                  ...
                  <TextArea
                    ...
                    id="category-keywords-textarea"
                    updateInput={(keys) => setCategoryKeywords(keys)}
                  />
                ...
                <div className="admin-filters-c">
                  <label
                    htmlFor="category-filters-input"
                    className="form-label"
                  >
                    Category Filters:
                  </label>
                ...
                {actionType == "add" && (
                  <button className="submit-add-btn submit-btn" type="submit">
                    <MdOutlineLibraryAdd className="add-icon" /> Add Category
                  </button>
                )}
                {actionType == "update" && (
                  <button
                ...
                <button
                  type="submit"
                  className="submit-delete-btn submit-btn"
                  onClick={() => handleDeleteCategory(categoryName)}
                >
                  <AiFillDelete className="update-icon" /> Delete
                </button>
              </form>
            )}
          </div>
              ...
              <div className="product-c">
                ...
                <div className="product-info">
                  ...
                  <div className="product-rating">
                    {0} <span className="product-rating-count">({0})</span>
                  </div>
