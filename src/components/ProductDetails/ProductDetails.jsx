...
function ProductDetails() {
    ...
    useEffect(()=>{
        if(user && recent){
            ...
            const items = [...recent].map(el=>el._id).filter(id=>id!==productId)
        ...
        <div className="product-details-page">
            <div className="product-details-body">
                <div className="product-details-img-c">
                    <img className='product-details-img' src={detailsData?.product?.image} alt="" />
                </div>
                <div className="product-details-c">
                    <h3 className='product-details-title'>{detailsData?.product?.title}</h3>
                    <a href="" className="rating-score-c"></a>
                    <div className="product-details-desc-c">
                        <div className="product-details-desc">{detailsData?.product?.description}</div>
                    ...
                    <form className="review-form" onSubmit={(e)=>SubmitReview(e)}>
                        <div className="review-c" >
                            <div className="review-user-c">
                                <div className="review-user-profile"><AiOutlineUser/></div>
                                <div className="review-user-info">
                                    <div className="rating">
                                        {new Array(5).fill(0).map((x,index)=>(
                                            <button className='review-stars-c' type='button' onClick={()=>setReviewRating(index+1)}>
                                                <AiFillStar className={index<reviewRating?"yellow review-star":"grey review-star"} />
                                            </button>
                                        ))}
                                    </div>
                            ...
                            <TextArea placeholder='Review item ...' className='mt2 w-100' updateInput={(text)=>setReviewBody(text)} />
                        ...
                        <li key={index} className="review-c">
                            <div className="review-user-c">
                                ...
                                <div className="review-user-info">
                                    ...
                                    <div className="review-username">{review.user} 
                            ...
                            {review._id === existingReview &&
                                <button className="delete-review-btn" onClick={()=>DeleteReview(review._id)}><RiDeleteBack2Line className='delete-review-icon'/></button>}
                        </li>
                    ))}
                </ul>
                {detailsData?.reviews.length==0 && <h3 className='no-reviews-heading'>Looks like theres no reviews yet!</h3>}
            </div>
        </div>
     );
}
